...
import me.denagon.database.sqlite.SQLite;
        ...
        if (getExact(columns.get(0)) == null) {
            ...
            try {
                ...
                for (int i = 0; i < columns.size(); i++) {
                    ...
                    } else if (columns.get(i).dataType == DataType.FLOAT) {
                        s.setFloat(i + 1, Float.parseFloat(columns.get(i).getValue().toString()));
                    } else if (columns.get(i).dataType == DataType.BOOLEAN) {
                    	s.setBoolean(i + 1, Boolean.parseBoolean(columns.get(i).getValue().toString()));
                    } // TODO Finish Adding Boolean
                }
                s.executeUpdate();
                s.close();
